...
class TagDatabase:
    ...
    def _check_if_tag_exists(self, tag: str) -> bool:
        if self.db.execute(
                select([self.tag_groups.c.group_tag]).where(self.tag_groups.c.group_tag == tag)).first() is not None:
            return True
        else:
            return False
    ...
    def insert_user_membership(self, tag: str, user_id: str):
        tag_id = self.db.execute(select([self.tag_groups.c.tg_id]).where(self.tag_groups.c.group_tag == tag)).first()
